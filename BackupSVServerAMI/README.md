...
```bash
BackupSVServerAMI$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
BackupSVServerAMI$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
BackupSVServerAMI$ dotnet test test/BackupSVServerAMI.Tests/BackupSVServerAMI.Tests.csproj 
```
...
```bash
sam delete --stack-name BackupSVServerAMI
```
