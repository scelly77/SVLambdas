...
```bash
CleanOldImages$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
CleanOldImages$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
CleanOldImages$ dotnet test test/CleanOldImages.Tests/CleanOldImages.Tests.csproj 
```
...
```bash
sam delete --stack-name CleanOldImages
```
